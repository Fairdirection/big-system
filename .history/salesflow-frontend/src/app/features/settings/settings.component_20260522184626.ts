...
import {
  ...
  getAvailableYears,
  getQuarterId,
} from "@core/utils/quarter.utils";
import { ConfirmDialogService } from "@core/services/confirm-dialog.service";
import { TranslateModule, TranslateService } from "@ngx-translate/core";
import { LanguageService } from "@core/services/language.service";
import { BackupManagementComponent } from "./components/backup-management.component";
  ...
  template: `
    <div class="space-y-8 animate-fade-in pb-20">
      ...
      >
        <div>
          <h1
            class="text-3xl font-display font-bold text-sf-text tracking-tight"
          >
      ...
      <div
        *ngIf="saveSuccessMessage()"
        class="fixed top-24 left-6 z-[110] bg-sf-success text-white px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 animate-slide-in-left border border-white/20"
      >
      ...
      <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div
          class="lg:col-span-1 lg:h-fit flex lg:flex-col gap-2 overflow-x-auto pb-2 lg:pb-0 scrollbar-none border-b border-sf-border/30 lg:border-b-0 lg:bg-sf-surface/80 lg:backdrop-blur-md lg:p-3 lg:rounded-3xl lg:border lg:border-sf-border/40 lg:shadow-lg"
        >
          <button
          ...
          <button
            (click)="activeTab.set('commissionRules')"
            [class]="
          ...
          <button
            ...
            "
          >
            <ng-icon name="heroArchiveBox" class="text-lg"></ng-icon>
            💾 Backup
          </button>
        </div>
        ...
        <div class="lg:col-span-3 space-y-8">
          ...
          <div
            *ngIf="activeTab() === 'lists'"
            class="space-y-6 animate-fade-in"
          >
            <!-- Segmented Switcher for Nested Lists -->
            <div
              class="flex items-center gap-1.5 p-1 bg-sf-surface border border-sf-border rounded-2xl overflow-x-auto scrollbar-none shadow-inner"
            >
              <button
                (click)="activeListSubTab.set('saleSource')"
                [class]="
                  activeListSubTab() === 'saleSource'
              ...
              <button
                (click)="activeListSubTab.set('collectionPercentage')"
                [class]="
                  activeListSubTab() === 'collectionPercentage'
                    ? 'flex-1 min-w-[120px] sm:min-w-0 px-4 py-3 bg-sf-primary text-white rounded-xl text-xs font-bold shadow-glow-sm whitespace-nowrap'
                    : 'flex-1 min-w-[120px] sm:min-w-0 px-4 py-3 text-sf-muted hover:text-sf-text rounded-xl text-xs font-bold transition-all whitespace-nowrap'
                "
              >
              ...
              <button
                (click)="activeListSubTab.set('invoiceType')"
                [class]="
                  activeListSubTab() === 'invoiceType'
                    ? 'flex-1 min-w-[120px] sm:min-w-0 px-4 py-3 bg-sf-primary text-white rounded-xl text-xs font-bold shadow-glow-sm whitespace-nowrap'
                    : 'flex-1 min-w-[120px] sm:min-w-0 px-4 py-3 text-sf-muted hover:text-sf-text rounded-xl text-xs font-bold transition-all whitespace-nowrap'
                "
              >
              ...
              <button
                (click)="activeListSubTab.set('tax')"
                [class]="
                  activeListSubTab() === 'tax'
                    ? 'flex-1 min-w-[120px] sm:min-w-0 px-4 py-3 bg-sf-primary text-white rounded-xl text-xs font-bold shadow-glow-sm whitespace-nowrap'
                    : 'flex-1 min-w-[120px] sm:min-w-0 px-4 py-3 text-sf-muted hover:text-sf-text rounded-xl text-xs font-bold transition-all whitespace-nowrap'
                "
              >
            ...
            <div
              class="glass-card p-6 sm:p-8 rounded-3xl border border-sf-border shadow-2xl space-y-6 text-right animate-fade-in"
            >
              <!-- Sale Sources Container -->
              <div
                *ngIf="activeListSubTab() === 'saleSource'"
                class="space-y-6"
              >
                <div
                  class="flex items-center justify-between pb-4 border-b border-sf-border/30"
                >
                  <div>
                    <h3 class="text-lg font-display font-bold text-sf-text">
                      مصادر المبيعات
                    </h3>
                    <p class="text-xs text-sf-muted mt-1">
                      تحديد القنوات التسويقية ومصادر جلب العملاء الجدد.
                    </p>
                  </div>
                  <button
                    (click)="openModal('saleSource')"
                    class="text-xs font-black text-sf-primary uppercase tracking-widest flex items-center gap-2 bg-sf-primary/5 hover:bg-sf-primary/10 px-4 py-2.5 rounded-xl transition-all shadow-sm"
                  >
                  ...
                  @for (s of getSettingsByType("saleSource"); track s._id) {
                    ...
                    >
                      <div class="flex items-center gap-3">
                        <div
                          class="w-2.5 h-2.5 rounded-full bg-sf-primary"
                        ></div>
                        <span class="text-sm font-semibold text-sf-text">{{
                          s.label
                        }}</span>
                      </div>
                      <div class="flex items-center gap-3">
                        <span
                          class="text-[10px] font-bold text-sf-muted uppercase tracking-wider bg-sf-surface border border-sf-border px-2 py-0.5 rounded-md"
                          >{{ s.value }}</span
                        >
                        <button
                          (click)="deleteSetting(s._id)"
                          class="opacity-100 lg:opacity-0 group-hover:opacity-100 p-1.5 text-sf-muted hover:text-sf-danger hover:bg-sf-danger/10 rounded-lg transition-all"
                        >
                    ...
                    <div
                      class="col-span-full py-12 flex flex-col items-center justify-center text-sf-muted"
                    >
                      <ng-icon
                        name="heroSwatch"
                        class="text-3xl opacity-30 mb-2"
                      ></ng-icon>
              ...
              <div
                *ngIf="activeListSubTab() === 'collectionPercentage'"
                class="space-y-6"
              >
                <div
                  class="flex items-center justify-between pb-4 border-b border-sf-border/30"
                >
                  <div>
                    ...
                    <p class="text-xs text-sf-muted mt-1">
                      النسب المئوية المستقطعة أو المعتمدة لتحصيل العمولات
                      والمستحقات.
                    </p>
                  </div>
                  <button
                    (click)="openModal('collectionPercentage')"
                    class="text-xs font-black text-sf-primary uppercase tracking-widest flex items-center gap-2 bg-sf-primary/5 hover:bg-sf-primary/10 px-4 py-2.5 rounded-xl transition-all shadow-sm"
                  >
                    <ng-icon name="heroPlus" class="text-sm"></ng-icon>
                    إضافة نسبة
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  @for (
                    ...
                    >
                      ...
                      <div class="flex items-center gap-3">
                        <span class="text-xs font-bold text-sf-info font-mono"
                          >{{ s.value }}%</span
                        >
                        <button
                          (click)="deleteSetting(s._id)"
              ...
              <div
                *ngIf="activeListSubTab() === 'invoiceType'"
                class="space-y-6"
              >
                <div
                  class="flex items-center justify-between pb-4 border-b border-sf-border/30"
                >
                  <div>
                    ...
                    <p class="text-xs text-sf-muted mt-1">
                      تحديد خيارات سداد الدفعات وأنواع عقود الفواتير المقبولة.
                    </p>
                  </div>
                  <button
                    (click)="openModal('invoiceType')"
                    class="text-xs font-black text-sf-primary uppercase tracking-widest flex items-center gap-2 bg-sf-primary/5 hover:bg-sf-primary/10 px-4 py-2.5 rounded-xl transition-all shadow-sm"
                  >
                    <ng-icon name="heroPlus" class="text-sm"></ng-icon>
                    إضافة طريقة سداد
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  @for (s of getSettingsByType("invoiceType"); track s._id) {
                    ...
                    >
                      <div class="flex items-center gap-3">
                        <div
                          class="w-2.5 h-2.5 rounded-full bg-sf-warning"
                        ></div>
                        <span class="text-sm font-semibold text-sf-text">{{
                          s.label
                        }}</span>
                      ...
                      <div class="flex items-center gap-3">
                        <span
                          class="text-[10px] font-bold text-sf-muted uppercase tracking-wider bg-sf-surface border border-sf-border px-2 py-0.5 rounded-md font-mono"
                          >{{ s.value }}</span
                        >
                        <button
                          (click)="deleteSetting(s._id)"
              ...
              <div *ngIf="activeListSubTab() === 'tax'" class="space-y-6">
                ...
                >
                  <div>
                    <h3 class="text-lg font-display font-bold text-sf-text">
                      الضرائب المتاحة في النظام
                    </h3>
                    <p class="text-xs text-sf-muted mt-1">
                      معدلات ونسب الضرائب المطبقة على عقود ومبيعات النظام.
                    </p>
                  </div>
                  <button
                    (click)="openModal('tax')"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                  @for (s of getSettingsByType("tax"); track s._id) {
                    ...
                    >
                      <div class="flex items-center gap-3">
                        <div
                          class="w-2.5 h-2.5 rounded-full bg-sf-danger"
                        ></div>
                        <span class="text-sm font-semibold text-sf-text">{{
                          s.label
                        }}</span>
                      ...
                      <div class="flex items-center gap-3">
                        <span class="text-xs font-bold text-sf-danger font-mono"
                          >{{ s.value }}%</span
                        >
                        <button
                          (click)="deleteSetting(s._id)"
          ...
          <div
            *ngIf="activeTab() === 'regional'"
            class="space-y-8 animate-fade-in"
          >
            <section
              class="glass-card p-6 sm:p-8 rounded-3xl border border-sf-border shadow-2xl space-y-6 text-right"
            >
              <div class="pb-4 border-b border-sf-border/30">
                ...
                <p class="text-xs text-sf-muted mt-1">
                  تخصيص لغة النظام والعملة الافتراضية وصيغ العرض المفضلة لديك.
                </p>
              </div>

              <form
                [formGroup]="regionalForm"
                (ngSubmit)="saveRegionalSettings()"
                class="space-y-6"
              >
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div class="space-y-2">
                    <label
                      class="text-xs font-black text-sf-muted uppercase tracking-widest mr-1"
                      >اللغة المفضلة</label
                    >
                    <select
                  ...
                  <div class="space-y-2">
                    ...
                    >
                      <option value="Africa/Cairo">
                        توقيت القاهرة (UTC+02:00)
                      </option>
                      <option value="Asia/Riyadh">
                        توقيت مكة المكرمة (UTC+03:00)
                      </option>
                ...
                <div class="flex justify-start pt-4">
                  <button
                    type="submit"
                    [disabled]="regionalForm.invalid"
                    class="px-8 py-3.5 rounded-2xl bg-sf-primary text-white font-bold shadow-glow-purple hover:brightness-110 transition-all flex items-center gap-2"
                  >
          ...
          >
            ...
            >
              <div
                class="pb-4 border-b border-sf-border/30 flex items-center gap-3"
              >
                ...
                <div>
                  <h3 class="text-lg font-display font-bold text-sf-text">
                    {{ "settings.quarter_label" | translate }}
                  </h3>
                  <p class="text-xs text-sf-muted mt-0.5">
                    {{ "settings.quarter_section_desc" | translate }}
                  </p>
                </div>
              </div>
              ...
              <div class="space-y-4">
                ...
                <div class="relative">
                  ...
                  <div
                    class="pointer-events-none absolute inset-y-0 end-0 w-8 bg-gradient-to-l from-sf-bg/80 to-transparent rounded-r-xl"
                  ></div>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-3" dir="ltr">
                  @for (qId of quartersForYear(); track qId) {
                    <button
                      type="button"
                      (click)="selectQuarter(qId)"
                      [class]="
                        themeService.currentQuarter() === qId
                    ...
                    >
                      ...
                      @if (qId === todayQuarter) {
                        <span
                          class="absolute top-2 end-2 px-1.5 py-0.5 bg-emerald-500/15 text-emerald-400 border border-emerald-500/30 rounded-md text-[9px] font-black leading-none"
                        >
                      ...
                      <span
                        ...
                        [dir]="
                          langService.currentLang() === 'ar' ? 'rtl' : 'ltr'
                        "
                      >
                        {{
                      ...
                      <span
                        class="text-[10px] font-semibold opacity-60 text-center leading-tight"
                      >
                        {{
                          langService.currentLang() === "ar"
          ...
          <div
            *ngIf="activeTab() === 'commissionRules'"
            class="space-y-8 animate-fade-in text-right"
          >
            <section
              class="glass-card p-6 sm:p-8 rounded-3xl border border-sf-border shadow-2xl space-y-8"
            >
              <!-- Premium Section Header -->
              <div
                class="pb-6 border-b border-sf-border/30 flex flex-col md:flex-row md:items-center justify-between gap-4"
              >
                <div class="space-y-1">
                  <h3
                    class="text-2xl font-display font-black text-sf-text bg-gradient-to-l from-sf-primary to-sf-info bg-clip-text text-transparent flex items-center gap-2"
                  >
                    <ng-icon
                      name="heroBriefcase"
                  ...
                  <p class="text-xs text-sf-muted font-medium">
                    تعديل المستهدفات البيعية، فئات عمولة الشركة، وشرائح مبيعات
                    المطور لكل مستوى وظيفي بالشركة بكل دقة.
                  </p>
                </div>
                <div class="flex items-center gap-2">
                  <span
                    class="px-3.5 py-1.5 bg-sf-primary/10 text-sf-primary text-[10px] font-black rounded-xl uppercase tracking-wider border border-sf-primary/20 flex items-center gap-1.5 shadow-sm"
                  >
                    <span
                      class="w-1.5 h-1.5 rounded-full bg-sf-primary animate-pulse"
                    ></span>
                    لوحة المدير المالي
                  </span>
                </div>
              ...
              <div class="space-y-3">
                <label
                  class="text-xs font-black text-sf-muted uppercase tracking-widest mr-1"
                  >المستوى الوظيفي المستهدف بالتعديل</label
                >
                <div
                  ...
                  >
                    <div class="flex items-center justify-between w-full">
                      <span
                        ...
                        [class.text-sf-primary]="selectedSeniority() === role"
                        >{{ role }}</span
                      >
                      <ng-icon
                        ...
                        [class]="
                          selectedSeniority() === role
                            ? 'text-sf-primary animate-bounce'
                            : 'text-sf-muted/60'
                        "
                      ></ng-icon>
                    </div>
                    <div class="space-y-0.5">
                      <p class="text-xs font-black leading-tight">
                        {{ getRoleSimpleLabel(role) }}
                      </p>
                      <p
                        class="text-[9px] opacity-75 font-semibold leading-none"
                      >
                        {{ getRoleSubLabel(role) }}
                      </p>
                    </div>
                  </button>
                </div>
              ...
              <div
                class="p-4 sm:p-5 rounded-2xl bg-sf-bg/50 border border-sf-border/60 flex items-start gap-4 transition-all"
              >
                <div
                  class="p-3 rounded-xl bg-sf-primary/10 text-sf-primary shadow-sm flex items-center justify-center"
                >
                  <ng-icon
                ...
                <div class="space-y-1">
                  <h4 class="text-xs sm:text-sm font-black text-sf-text">
                    آلية الاحتساب لـ {{ getRoleLabel(selectedSeniority()) }}
                  </h4>
                  <p class="text-xs text-sf-muted leading-relaxed font-medium">
                    {{ getRoleExplanation(selectedSeniority()) }}
                  </p>
                </div>
              </div>
              ...
              <div
                *ngIf="commissionRules() && currentRoleRules"
                class="space-y-8 animate-fade-in"
              >
                <!-- Target Settings Section -->
                <div
                  class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-sf-surface/40 p-6 rounded-2xl border border-sf-border/50"
                >
                  <!-- Normal Targets (Except Team Leader) -->
                  <div
                    class="space-y-2"
                    *ngIf="selectedSeniority() !== 'TeamLeader'"
                  >
                    <label
                      class="text-xs font-black text-sf-muted uppercase tracking-widest mr-1"
                      >المستهدف الربع سنوي الافتراضي</label
                    >
                    <div class="relative flex items-center">
                      ...
                      />
                      <span
                        class="absolute left-3 px-2.5 py-1 bg-sf-surface border border-sf-border rounded-lg text-[10px] font-black text-sf-muted font-mono"
                        >EGP</span
                      >
                    </div>
                    <div class="flex items-center justify-between mt-1.5 px-1">
                      <p class="text-[10px] text-sf-muted font-medium">
                        المستهدف الأصلي قبل تعديله بالأيام:
                        <span class="font-bold text-sf-text font-mono"
                          >{{ currentRoleRules.target || 0 | number }} ج.م</span
                        >
                      </p>
                      <span
                        class="text-[11px] font-black text-sf-primary bg-sf-primary/10 px-2 py-0.5 rounded-lg border border-sf-primary/20 shadow-sm"
                      >
                  ...
                  >
                    <span
                      class="text-xs font-black text-sf-muted uppercase tracking-widest mr-1"
                      >المستهدف الربع سنوي لقائد الفريق</span
                    >
                    <div
                      class="p-3.5 rounded-xl bg-sf-primary/5 border border-sf-primary/15 text-sf-primary font-bold text-xs flex items-center gap-2 mt-1"
                    >
                      <ng-icon name="heroUsers" class="text-base"></ng-icon>
                      <span
                        >المستهدف يتم احتسابه ديناميكياً كمجموع لمستهدفات
                        فريقه.</span
                      >
                    </div>
                  </div>
                  ...
                  <div
                    class="space-y-2"
                    *ngIf="selectedSeniority() === 'Fresh'"
                  >
                    <label
                      class="text-xs font-black text-sf-muted uppercase tracking-widest mr-1"
                      >العمولة الثابتة لكل مليون</label
                    >
                    <div class="relative flex items-center">
                      <input
                        type="number"
                    ...
                    <p class="text-[10px] text-sf-muted mt-1 px-1 font-medium">
                      يصرف هذا المبلغ كعلاوة ثابتة لكل مليون مبيعات مغلقة بنجاح.
                    </p>
                  </div>
                </div>
                ...
                <div class="space-y-4" *ngIf="selectedSeniority() !== 'Fresh'">
                  <div class="flex items-center justify-between">
                    <h4
                      class="text-sm font-black text-sf-text flex items-center gap-2"
                    >
                    ...
                    <button
                      type="button"
                      (click)="addTier()"
                      class="text-xs font-black text-sf-primary hover:text-sf-primary/80 flex items-center gap-1.5 bg-sf-primary/5 px-3.5 py-2 rounded-xl border border-sf-primary/10 transition-all shadow-sm"
                    >
                  ...
                  >
                    <div
                      *ngFor="
                        let tier of currentRoleRules.companyTiers;
                        let i = index
                      "
                      class="p-5 rounded-2xl bg-sf-bg/40 border border-sf-border hover:border-sf-primary/30 hover:bg-sf-surface/20 transition-all flex flex-col justify-between gap-4 relative group shadow-sm"
                    >
                      <!-- Card Header -->
                      <div
                        class="flex items-center justify-between border-b border-sf-border/30 pb-3"
                      >
                        <span
                          class="text-xs font-black text-sf-primary bg-sf-primary/10 px-2.5 py-1 rounded-lg border border-sf-primary/20"
                          >الشريحة {{ i + 1 }}</span
                        >
                        <button
                          type="button"
                          (click)="removeTier(i)"
                          class="text-sf-muted hover:text-sf-danger p-1.5 hover:bg-sf-danger/10 rounded-lg transition-all"
                        >
                      ...
                      <div class="grid grid-cols-2 gap-3">
                        <div class="space-y-1">
                          <label
                            class="text-[10px] font-black text-sf-muted uppercase tracking-wider mr-0.5"
                            >من تحقيق %</label
                          >
                          <div class="relative flex items-center">
                            <input
                            ...
                            />
                            <span
                              class="absolute left-2.5 text-[10px] text-sf-muted font-bold"
                              >%</span
                            >
                          </div>
                        </div>
                      ...
                      <div class="space-y-1">
                        ...
                        <div class="relative flex items-center">
                          ...
                          />
                          <span
                            class="absolute left-3 text-[10px] text-sf-muted font-bold"
                            >ج.م</span
                          >
                        </div>
                      </div>
                    ...
                    <div
                      *ngIf="
                        !currentRoleRules.companyTiers ||
                        currentRoleRules.companyTiers.length === 0
                      "
                      class="col-span-full py-10 flex flex-col items-center justify-center text-sf-muted bg-sf-bg/20 rounded-2xl border border-dashed border-sf-border"
                    >
                      <ng-icon
                        name="heroSwatch"
                        class="text-3xl opacity-30 mb-2"
                      ...
                      <p class="text-xs font-bold">
                        لا يوجد شرائح معرفة حالياً. أضف شريحة جديدة للبدء.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div class="space-y-4" *ngIf="selectedSeniority() !== 'Fresh'">
                  <div class="flex items-center justify-between">
                    ...
                    <button
                      type="button"
                      (click)="addSlab()"
                      class="text-xs font-black text-sf-info hover:text-sf-info/80 flex items-center gap-1.5 bg-sf-info/5 px-3.5 py-2 rounded-xl border border-sf-info/10 transition-all shadow-sm"
                    >
                      <ng-icon name="heroPlus" class="text-xs"></ng-icon>
                      إضافة شريحة شخصية
                    </button>
                  ...
                  >
                    <div
                      *ngFor="
                        let slab of currentRoleRules.personalSlabs;
                        let i = index
                      "
                      class="p-5 rounded-2xl bg-sf-bg/40 border border-sf-border hover:border-sf-info/30 hover:bg-sf-surface/20 transition-all flex flex-col justify-between gap-4 relative group shadow-sm"
                    >
                      <!-- Card Header -->
                      <div
                        class="flex items-center justify-between border-b border-sf-border/30 pb-3"
                      >
                        <span
                          class="text-xs font-black text-sf-info bg-sf-info/10 px-2.5 py-1 rounded-lg border border-sf-info/20"
                          >شريحة شخصية {{ i + 1 }}</span
                        >
                        <button
                          type="button"
                          (click)="removeSlab(i)"
                      ...
                      <div class="space-y-3">
                        ...
                        <div
                          class="flex items-center gap-2 pt-2.5 border-t border-sf-border/30"
                          *ngIf="
                            selectedSeniority() !== 'Fresh' &&
                            selectedSeniority() !== 'TeamLeader'
                          "
                        >
                          <input
                            type="checkbox"
                          ...
                          <label
                            [for]="'check_slab_' + i"
                            class="text-[11px] font-black text-sf-muted cursor-pointer select-none leading-none"
                            >الربط التلقائي بشرائح مبيعات الشركة</label
                          >
                        </div>
                      </div>
                    </div>
                    ...
                    <div
                      *ngIf="
                        !currentRoleRules.personalSlabs ||
                        currentRoleRules.personalSlabs.length === 0
                      "
                      class="col-span-full py-10 flex flex-col items-center justify-center text-sf-muted bg-sf-bg/20 rounded-2xl border border-dashed border-sf-border"
                    >
                      <ng-icon
                      ...
                      ></ng-icon>
                      <p class="text-xs font-bold">
                        لا يوجد شرائح شخصية معرفة حالياً. أضف شريحة للبدء.
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <!-- Submit Button -->
                <div
                  class="flex justify-start pt-6 border-t border-sf-border/30"
                >
                  <button
                    type="button"
                    (click)="saveCommissionRules()"
                    class="px-8 py-4 rounded-2xl bg-sf-primary text-white font-black shadow-glow-purple hover:brightness-110 active:scale-[0.98] transition-all flex items-center gap-2"
                  >
          ...
          >
            ...
            >
              <div class="pb-4 border-b border-sf-border/30">
                ...
                <p class="text-xs text-sf-muted mt-1">
                  تأكد من استخدام كلمة مرور قوية وغير مكررة لحماية حسابك.
                </p>
              </div>
            ...
            >
              ...
              <div class="space-y-6 divide-y divide-sf-border/30">
                ...
                <div class="flex items-center justify-between pt-4 first:pt-0">
                  <div class="space-y-1">
                    <h4 class="text-sm font-bold text-sf-text">
                      المصادقة الثنائية (Two-Factor Authentication)
                    </h4>
                    <p class="text-xs text-sf-muted">
                      تطلب إدخال رمز أمان مرسل إلى بريدك أو هاتفك عند تسجيل
                      الدخول الجديد.
                    </p>
                  </div>
                  <label
                  ...
                  >
                    ...
                    />
                    <div
                      class="w-11 h-6 bg-sf-border/50 peer-focus:outline-none rounded-full peer peer-checked:after:-translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:right-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-sf-primary"
                    ></div>
                  </label>
                </div>
                ...
                <div class="flex items-center justify-between pt-6">
                  <div class="space-y-1">
                    <h4 class="text-sm font-bold text-sf-text">
                      مدة بقاء الجلسة نشطة (Session Timeout)
                    </h4>
                    <p class="text-xs text-sf-muted">
                      يتم تسجيل الخروج تلقائياً في حال عدم وجود أي نشاط خلال
                      الفترة المحددة.
                    </p>
                  </div>
                  <select
                    (change)="updateSessionTimeout($event)"
                    class="px-4 py-2 bg-sf-bg border border-sf-border rounded-xl text-xs font-semibold focus:ring-2 focus:ring-sf-primary/50 outline-none transition-all text-sf-text"
                  >
                ...
                <div class="pt-6 space-y-4">
                  <div class="space-y-1">
                    ...
                    <p class="text-xs text-sf-muted">
                      قائمة بالأجهزة والمتصفحات التي قامت بتسجيل الدخول إلى
                      حسابك مؤخراً.
                    </p>
                  </div>
                  ...
                  <div class="space-y-3 mt-4">
                    <div
                      class="flex items-center justify-between p-4 bg-sf-bg/50 rounded-2xl border border-sf-border"
                    >
                      <div class="flex items-center gap-3">
                        <div
                          class="w-8 h-8 rounded-full bg-sf-primary/10 text-sf-primary flex items-center justify-center text-xs font-bold"
                        >
                          PC
                        </div>
                        <div class="space-y-0.5">
                          <span class="text-xs font-bold text-sf-text"
                            >Chrome on Windows (جهازك الحالي)</span
                          >
                          <span
                            class="text-[10px] text-sf-success font-semibold flex items-center gap-1"
                          >
                            <span
                              class="w-1.5 h-1.5 rounded-full bg-sf-success animate-pulse"
                            ></span>
                            نشط الآن
                          </span>
                        </div>
                      </div>
                      <span class="text-[10px] text-sf-muted font-bold"
                        >القاهرة، مصر</span
                      >
                    </div>

                    <div
                      class="flex items-center justify-between p-4 bg-sf-bg/30 rounded-2xl border border-sf-border/50"
                    >
                      <div class="flex items-center gap-3">
                        <div
                          class="w-8 h-8 rounded-full bg-sf-muted/10 text-sf-muted flex items-center justify-center text-xs font-bold"
                        >
                          MB
                        </div>
                        <div class="space-y-0.5">
                          <span class="text-xs font-bold text-sf-text"
                            >Safari on iPhone 15 Pro</span
                          >
                          <span class="text-[10px] text-sf-muted font-semibold"
                            >منذ ساعتين</span
                          >
                        </div>
                      </div>
          ...
          <div *ngIf="activeTab() === 'backup'" class="animate-fade-in">
      ...
      >
        <div
          class="absolute inset-0 bg-sf-bg/80 backdrop-blur-md"
          (click)="closeModal()"
        ></div>
        <div
          class="glass-card w-full max-w-md p-6 sm:p-8 rounded-[2rem] border border-sf-border shadow-2xl relative z-10 animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 text-right"
        >
          ...
          <form [formGroup]="form" (ngSubmit)="saveSetting()" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-4 mt-8">
              <button
                type="button"
                (click)="closeModal()"
                class="py-4 rounded-2xl bg-sf-surface border border-sf-border text-sf-text font-bold hover:bg-sf-bg transition-all font-semibold"
              >
                إلغاء
              </button>
              <button
                ...
                class="py-4 rounded-2xl bg-sf-primary text-white font-bold shadow-glow-purple hover:brightness-110 transition-all flex items-center justify-center gap-2"
              >
                <ng-icon name="heroCheck"></ng-icon>
...
export class SettingsComponent implements OnInit {
  ...
  activeTab = signal<
    "lists" | "regional" | "quarter" | "commissionRules" | "security" | "backup"
  >("lists");
  ...
  selectedYear = signal<number>(
    parseInt(this.themeService.currentQuarter().split("-")[1], 10),
  );
  readonly todayQuarter: string = getQuarterId(new Date());
  quartersForYear = computed<string[]>(() =>
    [1, 2, 3, 4].map((q) => `Q${q}-${this.selectedYear()}`),
  );
  ...
  constructor() {
    ...
    this.regionalForm = this.fb.group({
      language: [
        this.settingService.preferredLanguage(),
        [Validators.required],
      ],
      currency: [
        this.settingService.preferredCurrency(),
        [Validators.required],
      ],
      timezone: [
        this.settingService.preferredTimezone(),
        [Validators.required],
      ],
      dateFormat: [
        this.settingService.preferredDateFormat(),
        [Validators.required],
      ],
    });
  }
  ...
  saveSetting() {
    ...
    this.settingService.createSetting(data).subscribe({
      ...
      error: (err) => {
        this.showToast(
          "خطأ أثناء الحفظ: " + (err.error?.message || "حاول مرة أخرى"),
        );
      },
    });
  }
  ...
  async deleteSetting(id: string) {
    ...
    this.settingService.deleteSetting(id).subscribe({
      ...
      error: (err) =>
        this.showToast(
          "خطأ أثناء الحذف: " + (err.error?.message || "حاول مرة أخرى"),
        ),
    });
  }
  ...
  formatQ(q: string): string {
    return formatQuarter(
      q,
      this.langService.currentLang() === "ar" ? "ar" : "en",
    );
  }
  ...
  getRoleLabel(role: string): string {
    switch (role) {
      case "BA":
        return "مستشار عقاري (BA)";
      case "BC":
        return "مستشار عقاري أول (BC)";
      case "Senior":
        return "مستشار مبيعات كبار (Senior)";
      case "SV":
        return "مشرف مبيعات (SV)";
      case "TeamLeader":
        return "قائد فريق (Team Leader)";
      case "Fresh":
        return "موظف حديث التعيين (Fresh)";
      default:
        return role;
    }
  }
  ...
  getRoleSubLabel(role: string): string {
    switch (role) {
      case "BA":
        return "مبتدئ / عادي";
      case "BC":
        return "مبيعات متقدمة";
      case "Senior":
        return "مبيعات كبار";
      case "SV":
        return "إشراف ومتابعة";
      case "TeamLeader":
        return "إدارة فريق عمل";
      case "Fresh":
        return "تدريب وعمولة فلات";
      default:
        return "";
    }
  }
  ...
  saveCommissionRules() {
    ...
    const rulesSetting = this.settings().find(
      (s) => s.type === "commissionRules",
    );
    ...
    request$.subscribe({
      ...
      error: (err) => {
        this.showToast(
          "خطأ أثناء حفظ قواعد العمولات: " +
            (err.error?.message || "حاول مرة أخرى"),
        );
      },
    });
  }
