    ...
    <div class="space-y-6 animate-fade-in">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6"
        ...
        @for (emp of filteredEmployees(); track emp._id) {
          <div class="relative emp-card-frame rounded-2xl overflow-hidden cursor-pointer flex flex-col group"
            ...
            <!-- Header accent band -->
            <div class="emp-header-band absolute top-0 inset-x-0 h-20 pointer-events-none"></div>

            <!-- Rank accent strip (leading edge, RTL-safe) -->
            <div class="absolute start-0 top-0 bottom-0 w-[3px] emp-accent-strip"></div>
            ...
            <div class="relative z-20 p-5 flex flex-col gap-4">
              ...
              <div class="grid grid-cols-2 gap-x-4 gap-y-2 text-xs">
                <div class="flex items-center gap-1.5 min-w-0">
                  <ng-icon name="heroIdentification" class="text-sf-muted text-sm shrink-0"></ng-icon>
                  <span class="text-sf-muted truncate font-medium">{{ translateDepartment(emp.department) }}</span>
                </div>
                <div class="flex items-center gap-1.5 min-w-0">
                  <ng-icon name="heroCalendarDays" class="text-sf-muted text-sm shrink-0"></ng-icon>
                  <span class="text-sf-muted truncate font-medium">{{ emp.hireDate | date:'MMM yyyy' }}</span>
                </div>
                @if (emp.department === 'Sales') {
                  <div class="col-span-2 flex items-center gap-1.5 min-w-0 pt-1">
                    <ng-icon name="heroChartBar" class="text-sm shrink-0" [style.color]="'rgb(var(--emp-r))'"></ng-icon>
                    <span class="font-black text-sf-text truncate">{{ getEffectiveTarget(emp) | currencyEgp }}</span>
                    <span class="text-[9px] text-sf-muted shrink-0">{{ formatQ(currentQuarter()) }}</span>
                  </div>
                }
              </div>
              ...
              <div class="flex items-center justify-between pt-1">
                <div class="flex items-center gap-1.5">
                  <button class="p-1.5 bg-sf-bg border border-sf-border rounded-lg text-sf-muted hover:text-sf-primary hover:border-sf-primary/30 transition-all"
                ...
                <span class="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest group-hover:gap-2 transition-all shrink-0"
                      [style.color]="'rgb(var(--emp-r))'">
                  {{ 'employee.detail.edit_btn' | translate }}
                  <ng-icon name="heroChevronRight" class="rotate-180 text-xs"></ng-icon>
                </span>
              </div>
            </div>
          </div>
