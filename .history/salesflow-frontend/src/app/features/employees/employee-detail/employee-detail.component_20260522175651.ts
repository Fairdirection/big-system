...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, NgIconComponent, RouterLink, CurrencyEgpPipe, TranslateModule, AvatarFrameComponent],
  providers: [
    provideIcons({
      heroChevronLeft, heroChevronRight, heroChevronDown, heroPencil, heroTrash, heroEnvelope, heroPhone,
      heroCalendar, heroBriefcase, heroClock, heroIdentification, heroTrophy, heroChartBar,
  ...
  template: `
    ...
    @if (lightboxOpen() && employee()?.avatarUrl) {
      <div class="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-xl bg-black/50"
           (click)="lightboxOpen.set(false)">
        
        <div class="relative max-w-md w-full animate-fade-in" (click)="$event.stopPropagation()">
          <!-- Decorative background glow -->
          <div class="absolute inset-0 -z-10 rounded-3xl blur-3xl"
               style="background: radial-gradient(circle at center, rgba(var(--sf-primary),0.15) 0%, transparent 70%);"></div>
          ...
          <div class="glass-card rounded-3xl border border-sf-border shadow-2xl overflow-hidden">
            ...
            <div class="flex items-center justify-center py-12 px-8">
              <div class="relative">
                ...
                <div class="inline-block" style="transform: scale(2); transform-origin: center center;">
                  <app-avatar-frame
                    [level]="frameService.getFrameLevel(employee()!.seniorityLevel, employee()!.department)"
                    [variant]="frameService.getFrameVariant(employee()!.seniorityLevel, employee()!.department, employee()!.jobTitle)"
            ...
            <div class="px-8 pb-8 text-center space-y-3 border-t border-sf-border/50">
              <div class="pt-6">
                <h2 class="text-2xl font-display font-black text-sf-text">{{ employee()!.name }}</h2>
                @if (employee()!.jobTitle) {
                  <p class="text-sm font-bold text-sf-primary mt-1">{{ employee()!.jobTitle }}</p>
                }
                <p class="text-xs text-sf-muted font-semibold mt-2">{{ employee()!.department }}</p>
              </div>
            </div>
      ...
      <header class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div class="flex items-center gap-6">
          ...
          <div class="flex items-center gap-4 sm:gap-5 min-w-0"
            ...
            <div class="relative cursor-pointer group/av shrink-0"
              ...
              <div class="absolute inset-0 flex items-center justify-center rounded-full bg-black/40 opacity-0 group-hover/av:opacity-100 transition-opacity duration-200 pointer-events-none"
            ...
            <div class="min-w-0">
              ...
              <p class="text-xs sm:text-sm text-sf-muted font-bold flex flex-wrap items-center gap-2">
                ...
                <span class="emp-tier-chip inline-flex items-center px-1.5 py-0.5 rounded-md text-[9px] font-black border tracking-wider leading-none">
