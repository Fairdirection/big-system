...
async function testConcurrentCreation() {
  try {
    ...
    if (employees.length < 2) {
      console.log('❌ Need at least 2 active employees. Please seed the database first.');
      process.exit(1);
    }
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      salePromises.push(
        saleService.createSale(saleData)
          .then(sale => {
            console.log(`✅ Sale ${i}: Created ${sale.saleNumber}, Claim: ${sale._doc?.claimNumber || 'generating...'}`);
            return sale;
          })
          .catch(err => {
    ...
    if (claimNumbers.length === uniqueClaimNumbers.size) {
      ...
      const dupes = claimNumbers.filter((n, i) => claimNumbers.indexOf(n) !== i);
