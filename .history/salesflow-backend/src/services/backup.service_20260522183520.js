...
async function initBackupDirs() {
  try {
    await fs.mkdir(BACKUP_DB_DIR, { recursive: true });
    await fs.mkdir(BACKUP_FILES_DIR, { recursive: true });
  } catch (error) {
...
async function backupFiles(backupName) {
  ...
  try {
    ...
    try {
      await fs.access(filesSourceDir);
    } catch {
    ...
    const files = await fs.readdir(filesSourceDir, { recursive: true });
    let copiedCount = 0;
...
async function listBackups() {
  try {
    ...
    for (const manifest of manifests) {
      try {
        const content = await fs.readFile(path.join(BACKUP_DIR, manifest), 'utf-8');
  ...
  try {
    ...
    for (const backup of toDelete) {
      try {
        ...
        const manifestPath = path.join(BACKUP_DIR, `${backup.backupName}-manifest.json`);
        ...
        if (filesPath && await directoryExists(filesPath)) {
          await fs.rm(filesPath, { recursive: true, force: true });
        }
        ...
        console.warn(`Could not delete backup ${backup.backupName}: ${err.message}`);
      }
    }
...
async function getDirectorySize(dirPath) {
  try {
    const { stdout } = await execAsync(`dir /s "${dirPath}" | find "bytes"`, { shell: true });
    return stdout;
  } catch {
    return '0 bytes';
  }
